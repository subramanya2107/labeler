  ...
  if(IsNetscape() || (IsMicrosoft() && (bversion>=4.5)) || IsSafari() || IsChrome()) {
    ...
    if(video_mode) {
      ...
      function main_media_onload_helper() {
          ...
          var anno_file = main_media.GetFileInfo().GetFullName();
          anno_file = FetchImageAnnotationFileURL + anno_file.substr(0,anno_file.length-4) + '.xml' + '?' + Math.random();
          ReadXML(anno_file,LoadAnnotationSuccess,LoadAnnotation404);
      ...
      // Get the image:
      ReadJSON(FetchProjectTypeInformation+main_media.GetFileInfo().GetDirName()+".json"+ '?' + Math.random(),function(data){
    	  		main_media.GetNewImage(main_media_onload_helper);
    	  		projectTypeGlobalObj=data.projecttype;
  			if(data.projecttype=="1" || data.projecttype=="2"){
  				var anno_file = main_media.GetFileInfo().GetFullName();
  	            anno_file = FetchImageClassificationFileURL + anno_file.substr(0,anno_file.length-4) + '.json' + '?' + Math.random();
  				ReadJSON(anno_file,function(selecteddata){
  					LoadClassificationView(data,selecteddata)
  				},function(err){
  					LoadClassificationView(data,null)
  				});
  			}
      },function(err){
    	  		main_media.GetNewImage(main_media_onload_helper);
...
function LoadClassificationView(projectdata,selecteddata){
	    var preselected=[]
	    if(selecteddata && selecteddata.objects)
	    		preselected=selecteddata.objects;
		$("#anno_anchor").hide();
		$("#classification_options").show();
		var labels=projectdata.objects.split(",")
		var myvar = '<form action="'+SubmitClassificationUrl+'" method="POST">';
		
		if(projectdata.projecttype=="2")
			myvar+='<select name="objects" multiple="multiple" style="width:90%">';
		else{
			myvar+='<select name="objects" style="width:90%">';
			myvar+= ' <option value="">Select here</option>';
		}
			
		for(var i=0;i<labels.length;i++){
			if(preselected.indexOf(labels[i])!=-1){
				myvar+='  <option value="'+labels[i]+'" selected="true" >'+labels[i]+'</option>';
			}else
				myvar+='  <option value="'+labels[i]+'" >'+labels[i]+'</option>';
		
		}
			

		myvar+='</select> ';
		myvar+=' <input type="hidden"  name="filename" value="'+main_media.GetFileInfo().GetImName()+'" />';
		myvar+=' <input type="hidden"  name="foldername" value="'+main_media.GetFileInfo().GetDirName()+'" />';

		myvar+='<input type="submit" /> '+
		'</form>';
		$("#classification_options").html(myvar)	
}
...
function InitializeAnnotationTools(tag_button, tag_canvas){
    ...
    if(projectTypeGlobalObj=="1" || projectTypeGlobalObj=="2" ){
    		$('#polygonDiv').hide()
    }
    else if(projectTypeGlobalObj=="3"){
		$('#bounding_box').hide()
    }else if(projectTypeGlobalObj=="4"){
    		$('#polygon').hide()
    }
}
